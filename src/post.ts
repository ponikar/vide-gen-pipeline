...
async function main(): Promise<void> {
	...
	if (args.length < 2) {
		console.error(
			"Usage: npm run post -- <provider> <video_file> [caption...]",
		);
		console.error(
			'       npm run post -- instagram out/video.mp4 "My caption"',
		);
		process.exit(1);
	}
	...
	const { uploadUrl, publicUrl } = await fetch(
		`${BASE_URL}/api/content/upload-url`,
		{
			method: "POST",
	...
	if (result.permalink) {
		...
		for (let attempt = 0; attempt < POLL_RETRIES; attempt++) {
			...
			const status = await fetch(
				...
				(r) => r.json() as Promise<{ status: string; errorMessage?: string }>,
			);
