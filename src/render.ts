...
async function renderChatVideo(options: RenderOptions): Promise<void> {
	...
	const frames = await renderChatFrames(
		options.segments,
		options.chatConfig,
		options.tempDir,
	);
	...
	await runCommand("ffmpeg", [
		...
		"2:a:0",
		"-t",
		frames.totalDuration.toFixed(3),
		"-threads",
		"1",
		"-c:v",
		"libx264",
...
async function renderSubtitlesVideo(options: RenderOptions): Promise<void> {
	...
	const captionImagePaths = await createCaptionImages(
		captions,
		options.tempDir,
	);
...
async function concatAudioFiles(
	...
	const audioConcatContent = segments
		.map((s) => `file '${escapeConcatPath(s.audioPath)}'`)
		.join("\n");
...
async function finalizeVideo(
	...
	const videoConcatContent = segmentVideoPaths
...
async function createCaptionImages(
	...
	for (const caption of captions) {
		const captionImagePath = path.join(
			tempDir,
			`caption-${caption.index.toString().padStart(4, "0")}.png`,
		);
