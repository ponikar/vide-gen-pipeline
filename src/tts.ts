...
export async function generateSpeechSegments(
	...
	if (kokoroChunks.length > 0) {
		const parts = await generateKokoroSegments(
			kokoroChunks,
			voices,
			speed,
			tempDir,
	...
	if (cloneChunks.length > 0) {
		const parts = await generateCloneBatches(
			cloneChunks,
			voices,
			speed,
			tempDir,
...
async function ensureCustomVoices(tts: KokoroInstance): Promise<Set<string>> {
	...
	(tts as Record<string, unknown>)._validate_voice = (voiceName: string) =>
		voiceName[0];
