...
/* simple inline icons — stroke = currentColor, no fills, grayscale */
const IconCycle = () => (
...
// slight organic stagger (left indent per node)
...
export default function HowItWorks() {
  ...
  useEffect(() => {
    ...
    // reduced motion — show everything, no looping animation
    ...
    // self-running loop: draw node, then its connector, ~400ms apart;
    // hold 2s once all 4 are in, then reset and replay.
    const tl = gsap.timeline({ repeat: -1, repeatDelay: 2, paused: true });
    nodes.forEach((node, i) => {
      if (i > 0) tl.to({}, { duration: 0.4 }); // ~400ms between nodes
    ...
    // play while the section overlaps the middle band of the viewport, pause when it
    // leaves. Using a centered rootMargin (not an area threshold) keeps this working
    // when the stacked mobile layout is taller than the viewport — a fixed threshold
    // like 0.35 can never be met on a section >~3x viewport height, leaving the
    // opacity:0 nodes stuck invisible.
  ...
  return (
    <section className="hiw" id="how" ref={rootRef}>
      <div className="hiw-inner">
        ...
        {/* RIGHT: auto-running pipeline of compact icon nodes */}
