...
- Configured the same CommonJS Transformers instance used internally by `kokoro-js` to cache models in `KOKORO_CACHE_DIR` or `~/.cache/gold-fish/transformers`, outside replaceable pnpm `node_modules`. Added `@huggingface/transformers` as a direct dependency because runtime code now configures it explicitly.
