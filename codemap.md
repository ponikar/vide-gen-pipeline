...
Storage: Rendered MP4s upload to the configured Supabase Storage bucket. Local filesystem outputs are temporary.
Landing Page (Root /)
...
- Video source removed for open sourcing; landing renders CSS demo placeholders instead of bundled MP4 files.
...
- 4 cards in a grid. Cards now use CSS demo placeholders instead of checked-in video files.
...
Bundled public video files were removed before open sourcing. The landing page now renders CSS demo placeholders instead of checked-in MP4 assets.
...
8. prompts/videos-context.md has placeholder background MP4 URLs that should be replaced per deployment
Key Commits
...
- Old placeholder videos were removed before open sourcing.
...
- Root cause: video URLs are selected in `apps/agent-worker` and sent unchanged to video-server. A previous hardcoded Supabase fallback returned `400 application/json`. If clip extraction produced an empty list, video-server failed in `resolveVideoSource()`.
...
Open Source Sanitization (2026-08-16)
- Confirmed `origin` points to the intended GitHub repository.
- Genericized owned deployment/storage references in docs and examples, made TikTok callback URL env-driven via `TIKTOK_REDIRECT_URI`, and replaced background clip catalogs with placeholder public MP4 URLs.
- Removed tracked browser logs, TikTok verification artifact, bundled landing/demo MP4s, and generated voice binaries from the current tree. The landing page now renders CSS demo placeholders instead of bundled videos.
- Files modified in this iteration: `.env.example`, `README.md`, `apps/web-app/src/env.ts`, `apps/web-app/src/lib/tiktok/auth.ts`, `apps/web-app/src/components/landing-redesign/LandingPage.js`, `apps/web-app/src/styles/landing-redesign.css`, `prompts/video.md`, `prompts/videos-context.md`, `scripts/blend-voices.ts`, `src/config.ts`, `video-urls.md`, deleted tracked artifacts under `.gstack/`, `apps/web-app/public/`, and `voices/`, and `codemap.md`.
- Verification: old owned URL/project/path scan passes with no matches; provider-token prefix scan passes with no matches; `pnpm --filter @attentionspam/web typecheck`, `pnpm --filter @attentionspam/video-server typecheck`, and `git diff --check` pass. Root `pnpm test` is blocked by the existing root Vitest alias issue for `@/logger`; root `pnpm typecheck` and agent-worker typecheck are blocked by the existing missing `@ai-sdk/provider` type dependency.
