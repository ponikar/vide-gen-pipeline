...
const SCRAPE_YES = ["yes", "yep", "correct", "right", "looks good", "looks right"];
...
export default function OnboardPage() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: nextId(), role: "bot", content: "Let's get your app set up.\n\nDo you have a link to your app's website or App Store page? Paste it here, or type \"skip\" if you don't have one." },
  ]);
  const [step, setStep] = useState<Step>("INITIAL");
  const [collected, setCollected] = useState<Collected>({ name: "", description: "" });
  ...
  const handleSend = (value: string) => {
    ...
    switch (step) {
      ...
      case "REVIEW_SCRAPED": {
        ...
        if (isYes) {
          advanceTo("CREATING");
          createApp.mutate({ name: collected.name, description: collected.description || undefined });
          addBotMessage("Creating your app...");
        } else if (isNo) {
          addBotMessage("Let's fill in the details yourself.");
          advanceTo("ASKING_NAME");
        } else {
          addBotMessage('Please type "yes" to confirm or "no" to fill in manually.');
        }
        break;
      }
      ...
      case "ASKING_NAME": {
        ...
        addBotMessage(`Got it! Now tell me -- what does ${trimmed} do? Give me a short description.`);
      ...
      <div className="sticky bottom-0 border-t bg-background pt-4">
        <div className="flex gap-2">
          <input
            ...
            disabled={step === "SCRAPING" || step === "CREATING" || step === "DONE"}
          ...
          <button
            ...
            disabled={!input.trim() || step === "SCRAPING" || step === "CREATING" || step === "DONE"}
